...
function setupEventListeners() {
    ...
    if (navToggle && navLinksContainer) {
        navToggle.addEventListener('click', () => {
            const opened = navLinksContainer.classList.toggle('open');
...
function renderAdminAnalytics() {
    ...
    const conversionFunnelEl = document.getElementById('conversion-funnel');
    if (!conversionFunnelEl) return;
    conversionFunnelEl.innerHTML = `
...
function updateCartUI() {
    ...
    if (!cartItems) return; // No cart modal on this page

    if (state.cart.length === 0) {
        cartItems.innerHTML = '<div class="empty-cart">Your cart is empty</div>';
        if (checkoutBtn) checkoutBtn.disabled = true;
        if (cartSummary) {
            const totalEl = cartSummary.querySelector('#total');
            const subtotalEl = cartSummary.querySelector('#subtotal');
            const taxEl = cartSummary.querySelector('#tax');
...
window.addEventListener('load', () => {
    ...
    if (document.getElementById('product-chart')) renderProductChart();
    if (document.getElementById('stat-revenue')) renderAdminAnalytics();
});
